...
class WXRImporter extends WP_Importer {
	...
	private function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'quick-demo-import' ) . '</strong><br />';
			echo esc_html__( 'The file does not exist, please try again.', 'quick-demo-import' ) . '</p>';
			$this->footer();
			die();
		}
	...
	private function import_end() {
		...
		echo '<p>' . esc_html__( 'All done.', 'quick-demo-import' ) . ' <a href="' . esc_url( admin_url() ) . '">' . esc_html__( 'Have fun!', 'quick-demo-import' ) . '</a></p>';
		echo '<p>' . esc_html__( 'Remember to update the passwords and roles of imported users.', 'quick-demo-import' ) . '</p>';
	 ...
	 */
	private function get_authors_from_import( $import_data ) {
		if ( ! empty( $import_data['authors'] ) ) {
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					...
					printf( esc_html__( 'Failed to import author %s. Their posts will be attributed to the current user.', 'quick-demo-import' ), esc_html( $post['post_author'] ) );
					echo '<br />';
					continue;
				}
			...
			if ( ! empty( $_POST['user_map'][ $i ] ) ) {
				...
				if ( ! empty( $_POST['user_new'][ $i ] ) ) {
					...
					$user_data = [
						...
						'first_name'   => $this->authors[ $old_login ]['author_first_name'] ?? '',
						'last_name'    => $this->authors[ $old_login ]['author_last_name'] ?? '',
					];
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					printf( esc_html__( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'quick-demo-import' ), esc_html( $this->authors[ $old_login ]['author_display_name'] ) );
	...
	private function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) && $id > 0 ) {
				...
				printf( esc_html__( 'Failed to import category %s', 'quick-demo-import' ), esc_html( $cat['category_nicename'] ) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
	...
	private function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				/* Translators: %s: Category nicename. */
				printf( esc_html__( 'Failed to import post tag %s', 'quick-demo-import' ), esc_html( $tag['tag_name'] ) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
					echo ': ' . esc_html( $id->get_error_message() );
				}
				echo '<br />';
	...
	private function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( esc_html__( 'Failed to import %1$s %2$s', 'quick-demo-import' ), esc_html( $term['term_taxonomy'] ), esc_html( $term['term_name'] ) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
					echo ': ' . esc_html( $id->get_error_message() );
				}
				echo '<br />';
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				...
				/* Translators: 1: Post title. 2: Post type */
					esc_html__( 'Failed to import &#8220;%1$s&#8221;: Invalid post type %2$s', 'quick-demo-import' ),
			...
			if ( $post_exists && get_post_type( $post_exists ) === $post['post_type'] ) {
				...
				printf( esc_html__( '%1$s &#8220;%2$s&#8221; already exists.', 'quick-demo-import' ), esc_html( $post_type_object->labels->singular_name ), esc_html( $post['post_title'] ) );
				echo '<br />';
				$comment_post_ID                                     = $post_id = $post_exists; // phpcs:disable
				...
				$author = $this->author_mapping[ $author ] ?? get_current_user_id();
				...
				if ( is_wp_error( $post_id ) ) {
					printf(
						...
						__( 'Failed to import %1$s &#8220;%2$s&#8221;', 'quick-demo-import' ),
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							printf( __( 'Failed to import %1$s %2$s', 'quick-demo-import' ), esc_html( $taxonomy ), esc_html( $term['name'] ) );
							if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
	...
	private function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			_e( 'Menu item skipped due to missing menu slug', 'quick-demo-import' );
			echo '<br />';
		...
		if ( ! $menu_id ) {
			...
			printf( __( 'Menu item skipped due to invalid menu slug: %s', 'quick-demo-import' ), esc_html( $menu_slug ) );
			echo '<br />';
	...
	private function process_attachment( $post, $url ) {
		if ( ! $this->fetch_attachments ) {
			return new WP_Error(
				...
				__( 'Fetching attachments is not enabled', 'quick-demo-import' )
			);
		}
		...
		if ( $info = wp_check_filetype( $upload['file'] ) ) {
			...
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'quick-demo-import' ) );
			}
	...
	private function fetch_remote_file( $url, $post ) {
		...
		if ( ! $tmp_file_name ) {
			return new WP_Error( 'import_no_file', __( 'Could not create temporary file.', 'quick-demo-import' ) );
			}
		...
		if ( is_wp_error( $remote_response ) ) {
			...
			return new WP_Error(
				...
				/* translators: 1: The WordPress error message. 2: The WordPress error code. */
					__( 'Request failed due to an error: %1$s (%2$s)', 'quick-demo-import' ),
		...
		if ( 200 !== $remote_response_code ) {
			...
			return new WP_Error(
				...
				/* translators: 1: The HTTP error message. 2: The HTTP error code. */
					__( 'Remote server returned the following unexpected result: %1$s (%2$s)', 'quick-demo-import' ),
		...
		if ( ! $headers ) {
			...
			return new WP_Error( 'import_file_error', __( 'Remote server did not respond', 'quick-demo-import' ) );
			}
		...
		if ( 0 === $filesize ) {
			...
			return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'quick-demo-import' ) );
			}

		if ( ! isset( $headers['content-encoding'] ) && isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			@unlink( $tmp_file_name );

			return new WP_Error( 'import_file_error', __( 'Downloaded file has incorrect size', 'quick-demo-import' ) );
			}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			...
			return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s', 'quick-demo-import' ), esc_html( size_format( $max_size ) ) ) );
			}
		...
		if ( ( ! $type || ! $ext ) && ! current_user_can( 'unfiltered_upload' ) ) {
			return new WP_Error( 'import_file_error', __( 'Sorry, this file type is not permitted for security reasons.', 'quick-demo-import' ) );
			}
		...
		if ( ! $move_new_file ) {
			...
			return new WP_Error( 'import_file_error', __( 'The uploaded file could not be moved', 'quick-demo-import' ) );
			}
	...
	private function header() {
		echo '<div class="wrap">';
		echo '<h2>' . __( 'Import WordPress', 'quick-demo-import' ) . '</h2>';
		...
		if ( isset( $updates[ $basename ] ) ) {
			...
			printf( __( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'quick-demo-import' ), esc_html( $update->update->new_version ) );
			echo '</strong></p></div>';
			}
		}
	 ...
	 */
	private function allow_create_users() {
		return apply_filters( 'import_allow_create_users', true );
		}

	/**
	 * Decide whether the importer should attempt to download attachment files.
	 * Default is true, can be filtered via import_allow_fetch_attachments. The choice
	 * made at the import options screen must also be true, false here hides that checkbox.
	 *
	 * @return bool True if downloading attachments is allowed
	 */
	private function allow_fetch_attachments() {
