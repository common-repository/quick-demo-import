...
class WXRParserRegex {
	...
	public function parse( $file ) {
		$wxr_version = $in_multiline = false; // phpcs:disable
		...
		if ( ! $wxr_version ) {
			return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'quick-demo-import' ) );
		}
	...
	public function process_post( $post ) {
		...
		$post_excerpt = preg_replace_callback( '|<(/?[A-Z]+)|', [ &$this, 'normalize_tag' ], $post_excerpt );
		$post_excerpt = str_replace( '<br>', '<br />', $post_excerpt );
		$post_excerpt = str_replace( '<hr>', '<hr />', $post_excerpt );

		$post_content = $this->get_tag( $post, 'content:encoded' );
		$post_content = preg_replace_callback( '|<(/?[A-Z]+)|', [ &$this, 'normalize_tag' ], $post_content );
	...
	private function fgets( $fp ) {
		if ( $this->has_gzip ) {
			return gzgets( $fp, 8192 );
		}
