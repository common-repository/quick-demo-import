( function( $, wp ) {
	...
	wp.updates.importDemo = function( args ) {
		...
		if ( $message.html() !== __( 'Importing...', 'quick-demo-import' ) ) {
			$message.data( 'originaltext', $message.html() );
		}

		$message
			.text( __( 'Importing...', 'quick-demo-import' ) )
			.attr(
				'aria-label',
				sprintf(
					/* translators: %s: Demo name. */
					_x( 'Importing %s...', 'demo', 'quick-demo-import' ),
					$message.data( 'name' )
				)
			);
		wp.a11y.speak( __( 'Importing... please wait.', 'quick-demo-import' ), 'polite' );
	...
	wp.updates.importDemoSuccess = function( response ) {
		...
		$message = $card.find( '.button-primary' )
			...
			.attr(
				...
				sprintf(
					/* translators: %s: Demo name. */
					_x( '%s imported!', 'demo', 'quick-demo-import' ),
					response.demoName
				)
			)
			.text( __( 'Imported!', 'quick-demo-import' ) );

		wp.a11y.speak( __( 'Import completed successfully.', 'quick-demo-import' ), 'polite' );
			...
			if ( response.previewUrl ) {
				...
				$message
					...
					.attr(
						...
						sprintf(
							/* translators: %s: Demo name. */
							_x( 'Live Preview %s', 'demo', 'quick-demo-import' ),
							response.demoName
						)
					)
					.text( __( 'Live Preview', 'quick-demo-import' ) );
			}
		}, 1000 );
	};
	...
	wp.updates.importDemoError = function( response ) {
		var $card, $button,
			errorMessage = sprintf(
				...
				__( 'Import failed: %s', 'quick-demo-import' ),
		...
		$button
			...
			.attr(
				...
				sprintf(
					/* translators: %s: Demo name. */
					_x( '%s import failed', 'demo', 'quick-demo-import' ),
					$button.data( 'name' )
				)
			)
			.text( __( 'Import Failed!', 'quick-demo-import' ) );
	...
	wp.updates.bulkInstallPlugin = function( args ) {
		...
		if ( $document.find( 'body' ).hasClass( 'full-overlay-active' ) ) {
			...
			message    = sprintf(
				...
				_x( 'Installing %s...', 'plugin', 'quick-demo-import' ),
				$pluginRow.data( 'name' )
			);
			$pluginRow.find( '.plugin-status span' )
				...
				.text( __( 'Installing...', 'quick-demo-import' ) );
		} else {
		...
		if ( $message.html() !== __( 'Installing...', 'quick-demo-import' ) ) {
			$message.data( 'originaltext', $message.html() );
		}
		...
		wp.a11y.speak( __( 'Installing... please wait.', 'quick-demo-import' ), 'polite' );

		$document.trigger( 'wp-plugin-bulk-installing', args );

		return wp.updates.ajax( 'install-required-plugins', args );
	};
	...
	wp.updates.bulkInstallPluginSuccess = function( response ) {
		...
		$updateMessage
			...
			.attr(
				...
				sprintf(
					/* translators: %s: Plugin name. */
					_x( '%s installed!', 'plugin', 'quick-demo-import' ),
					response.pluginName
				)
			)
			.text( _x( 'Installed!', 'plugin', 'quick-demo-import' ) );

		wp.a11y.speak( __( 'Installation completed successfully.', 'quick-demo-import' ), 'polite' );
	...
	wp.updates.bulkInstallPluginError = function( response ) {
		...
		errorMessage = sprintf(
			...
			__( 'Installation failed: %s', 'quick-demo-import' ),
			response.errorMessage
		);
		...
		$updateMessage
			...
			.attr(
				...
				sprintf(
					/* translators: %s: Plugin name. */
					_x( '%s installation failed', 'plugin', 'quick-demo-import' ),
					$pluginRow.data( 'name' )
				)
			)
			.text( __( 'Installation Failed!', 'quick-demo-import' ) );
	...
	wp.updates.isValidResponse = function( response, action ) {
		var error = __( 'Something went wrong.', 'quick-demo-import' ),
			errorMessage;
		...
		if ( _.isString( response ) && '-1' === response ) {
			error = __( 'An error has occurred. Please reload the page and try again.', 'quick-demo-import' );
		} else if ( _.isString( response ) ) {
			error = response;
		} else if ( 'undefined' !== typeof response.readyState && 0 === response.readyState ) {
			error = __( 'Connection lost or the server is busy. Please try again later.', 'quick-demo-import' );
		} else if ( _.isString( response.statusText ) ) {
			// TODO: Update URL.
			error = response.statusText + ' ' + '<a href="https://quickdemoimport.com/#demo-import-process-failed" target="_blank">' + __( 'Try this solution!', 'quick-demo-import' ) + '</a>';
		}

		switch ( action ) {
			case 'import':
				errorMessage = __( 'Import failed: %s', 'quick-demo-import' );
				break;

			case 'install':
				errorMessage = __( 'Installation failed: %s', 'quick-demo-import' );
				break;
		}
